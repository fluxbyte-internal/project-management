...
export const authLoginSchema = z.object({
  email: z.string({required_error:ZodErrorMessageEnumValue.REQUIRED}).email({ message: "Email is not valid" }),
  password: z.string({required_error:ZodErrorMessageEnumValue.REQUIRED}).min(1, "Password is a required field"),
});

export const authRefreshTokenSchema = z.string();

export const verifyEmailOtpSchema = z.object({
  otp: z.string({required_error:ZodErrorMessageEnumValue.REQUIRED}).min(1, "Otp is required field"),
});

export const forgotPasswordSchema = z.object({
  email: z.string({required_error:ZodErrorMessageEnumValue.REQUIRED}).email(),
});
